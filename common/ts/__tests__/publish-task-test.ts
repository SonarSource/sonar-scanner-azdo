...
it('should fail unless SONARQUBE_SCANNER_PARAMS are supplied', async () => {
  ...
  expect(tl.setResult).toBeCalledWith(tl.TaskResult.Failed,
...
it('get report string for single report', async () => {
  ...
  const returnedTask = new Task({ analysisId: '123', componentKey: 'key', status: 'status', type: EndpointType.SonarQube, componentName: 'componentName' });
  jest.spyOn(Task, 'waitForTaskCompletion').mockImplementation(() => returnedTask);

  // Mock converting the Task into an html report
  const returnedAnalysis = new Analysis({ status: '', conditions: [] }, EndpointType.SonarCloud, '', null, null);
  ...
  expect(Analysis.getAnalysis).toBeCalledWith(
    { analysisId: '123', dashboardUrl: 'http://dashboardurl1', endpoint: SQ_ENDPOINT, metrics: { metrics: [] }, projectName: 'componentName' });
...
it('get report string should return undefined if ceTask times out', async () => {
  ...
  jest.spyOn(Task, 'waitForTaskCompletion').mockImplementation(() => { throw new TimeOutReachedError(); });
  ...
  expect(Task.waitForTaskCompletion).toHaveBeenCalledWith(SQ_ENDPOINT, TASK_REPORT.ceTaskId, 999);
  expect(tl.warning).toBeCalledWith('Task \'111\' takes too long to complete. Stopping after 999s of polling. No quality gate will be displayed on build result.');
  expect(Analysis.getAnalysis).not.toBeCalled();
});

it('get report string should fail for non-timeout errors', async () => {
  // Mock the ceTask timing out
  jest.spyOn(Task, 'waitForTaskCompletion').mockImplementation(() => { throw new InvalidApiResourceVersionError('my error'); });
  jest.spyOn(Analysis, 'getAnalysis');
  jest.spyOn(tl, 'warning').mockImplementation(() => null);
...
it('task should not fail the task even if all ceTasks timeout', async () => {
  ...
  jest.spyOn(TaskReport, 'createTaskReportsFromFiles').mockImplementation(() => [TASK_REPORT, taskReport2]);
  ...
  const publishSummaryMock = (serverUtils.publishBuildSummary as jest.Mock<any>);
  ...
  expect(tl.warning).toBeCalledWith('Task \'111\' takes too long to complete. Stopping after 1s of polling. No quality gate will be displayed on build result.');
  expect(tl.warning).toBeCalledWith('Task \'222\' takes too long to complete. Stopping after 1s of polling. No quality gate will be displayed on build result.');
});
