...
const { paths, DIST_DIR, resolveRelativePath } = require("./paths");
...
function cycloneDxPipe(flavour, packageJSON, ...ps) {
  ...
  return mergeStream(
    ...
    .pipe(
      exec(
        (file) =>
          `npm run cyclonedx-run -- --output ${file.dirname}/cyclonedx.json ${file.dirname}`,
      ),
    )
    .pipe(exec.reporter())
    .pipe(
    ...
    .pipe(
      collect.list((files) => {
        const mergeFile = new Vinyl({
          ...
          path: path.join(
            DIST_DIR,
            `${packageJSON.name}-${packageVersion}-${flavour}-cyclonedx.json`,
          ),
          contents: null,
        });
    ...
    .pipe(
      exec(
        (file) => `npm run cyclonedx-run -- merge \
    --hierarchical \
