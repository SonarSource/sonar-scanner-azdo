...
import * as publishTaskModule from "../publish-task";
...
it("should fail unless SONARQUBE_SCANNER_PARAMS are supplied", async () => {
  ...
  await publishTaskModule.publishTask(EndpointType.SonarCloud);
...
it("get report string should return undefined if ceTask times out", async () => {
  ...
  const result = await publishTaskModule.getReportForTask(TASK_REPORT, METRICS, SQ_ENDPOINT, 999);
...
it("get report string should fail for non-timeout errors", async () => {
  ...
  expect.assertions(1);
  try {
    await publishTaskModule.getReportForTask(TASK_REPORT, METRICS, SQ_ENDPOINT, 999);
  } catch (e) {
...
describe("it should generate passing report correctly", () => {
  ...
  it("not fail when measures can not be retrieved", async () => {
    ...
    await publishTaskModule.publishTask(EndpointType.SonarQube);
  ...
  it.each([
    ...
    async (endpointType, endpoint, scannerParams, shouldShow) => {
      ...
      await publishTaskModule.publishTask(endpointType);
